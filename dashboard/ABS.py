...
def display_home_page():
    ...
    # Create 3 columns for the albums
    columns = st.columns(3)

    # Iterate through the albums and display them in the columns
    for i, album in enumerate(data["albums"]):
        cover_path = os.path.join("Database/audiobooks", album["cover"])

        if file_exists(cover_path):
            with columns[i % 3]:  # Distribute albums across 3 columns
    ...
    for track in album["tracks"]:
        ...
        if file_exists(audio_path):
            ...
            if st.session_state.playing_track_id == track['id']:
                ...
                # Optionally reset the audio player after it finishes playing
...
def run():
# Main logic for handling page transitions
